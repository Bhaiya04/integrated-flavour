...
(() => {
  ...
  gl.bufferData(gl.ARRAY_BUFFER, positions, gl.STATIC_DRAW); gl.enableVertexAttribArray(posLoc);
  gl.vertexAttribPointer(posLoc, 2, gl.FLOAT, false, 0, 0);

  const uvLoc = gl.getAttribLocation(program, 'a_uv');
  const uvBuffer = gl.createBuffer(); gl.bindBuffer(gl.ARRAY_BUFFER, uvBuffer);
  gl.bufferData(gl.ARRAY_BUFFER, uvs, gl.STATIC_DRAW); gl.enableVertexAttribArray(uvLoc);
  ...
  function setMouseFromEvent(e) {
    ...
    mouse.x = x; mouse.y = y; gl.uniform2f(uMouseLoc, x, y);
  }
  if (MOUSE_REACT) {
    canvas.addEventListener('mousemove', setMouseFromEvent);
    canvas.addEventListener('touchmove', (ev) => {
      if (ev.touches && ev.touches.length) setMouseFromEvent(ev.touches[0]);
    }, { passive: true });
  }
  ...
  window.__Iridescence = {
    setColor: (r, g, b) => gl.uniform3f(uColorLoc, r, g, b),
    setAmplitude: v => gl.uniform1f(uAmplitudeLoc, v),
    setSpeed: v => gl.uniform1f(uSpeedLoc, v),
    dispose: () => { canvas.remove(); gl.getExtension('WEBGL_lose_context')?.loseContext(); }
  };

  document.getElementById('year').textContent = new Date().getFullYear();
  document.getElementById('demoToggle')?.addEventListener('change', e => {
    document.querySelector('.hero-inner').style.opacity = e.target.checked ? '0.12' : '1';
  });
})();